...
public class DisparoPlayerNivel2 : MonoBehaviour
{
  ...
  void Update()
    ...
    void DispararEnV()
    {
        ...
        DispararConAngulo(anguloIzq);
        DispararConAngulo(anguloDer);
        DispararConAngulo(0f);
    }

    void DispararConAngulo(float angulo)
    {
        GameObject bala1 = Instantiate(bala, firePoint.position, Quaternion.identity);
        Rigidbody2D rb = bala1.GetComponent<Rigidbody2D>();
        // Convertir el ángulo a dirección
